...
from m2k_streamout_utils import SamplingParams, generate_buffer

# serial shift register clock parameters
...
print("clock frequency/bit rate {} kHz".format(CLOCK_FREQUENCY / 1000))
print("samples per period: {} ".format(samples_per_period))
print("buffer size: {} kSamples".format(buffer_size / 1000))
...
print(
    "ENABLE IO channels [w, rd]: {}".format(
        [DCH_W_ENABLE, DCH_RD_ENABLE]
    )
)
print(
    "CLOCK IO channels [w, rd]: {}".format(
        [DCH_W_CLK, DCH_RD_CLK]
    )
)
print(
    "DATA  IO channels [w, rd]: {}".format(
        [DCH_W_DATA, DCH_RD_DATA]
    )
)
...
digital_enable_read = list(
    map(
        lambda s: ((0x0001 << DCH_RD_ENABLE) & int(s))
        >> DCH_RD_ENABLE, digital_data
    )
)
digital_clk_read = list(
    map(
        lambda s: ((0x0001 << DCH_RD_CLK) & int(s))
        >> DCH_RD_CLK, digital_data
    )
)
digital_data_read = list(
    map(
        lambda s: ((0x0001 << DCH_RD_DATA) & int(s))
        >> DCH_RD_DATA, digital_data
    )
)
time.sleep(pause)
libm2k.contextClose(ctx, True)
print("plotting input channels...")

OFFSET = 1.5
time_step = 1 / digital_sampling_freq
stop_time = buffer_size * time_step
# print(time_step, stop_time)
